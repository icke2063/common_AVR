...
void DS18X20_show_id_uart( uint8_t *id, size_t n )
{
	...
	for( i = 0; i < n; i++ ) {
	    if ( i == 0 ) uart_puts( "FC:" );
		else if ( i == n-1 ) uart_puts( "CRC:" );
		if ( i == 1 ) uart_puts( "SN: " );
		usart_write("%x ",id[i]);
		if ( i == 0 ) {
			if ( id[0] == DS18S20_ID ) uart_puts ("(18S)");
			else if ( id[0] == DS18B20_ID ) uart_puts ("(18B)");
			else uart_puts ("( ? )");
		}
	}
...
uint8_t DS18X20_read_meas(const uint8_t id[], uint8_t *subzero,
